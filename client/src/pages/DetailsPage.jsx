  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setData({
          content: result.content || result.title, // Use title for Reddit
          media_url: result.media_url || result.thumbnail_url,
          timestamp: result.timestamp || result.created_time,
          comments: result.comments || result.num_comments,
          like_count: result.like_count || result.ups,
        ...
        const currentPost = mergedPosts.find((post) => {
          if (platform === "facebook") {
            ...
            return post.reddit && post.reddit.id === id;
          }
          return false;
        });
        ...
        if (currentPost) {
          const related = mergedPosts.find((post) => {
            if (platform === "facebook") {
              ...
              return (
                (post.facebook &&
                  post.facebook.message === currentPost.reddit.title) ||
                (post.instagram &&
                  post.instagram.caption === currentPost.reddit.title)
              );
            }
            return false;
          });
          ...
          <div className="post-details-image-container">
            ...
            <div className="details-buttons">
              {platform !== "instagram" && platform !== "reddit" ? (
                ...
                <p>
                  {platform === "instagram"
                    ? "Instagram does not support deleting posts with API."
                    : "Reddit does not support deleting posts with API."}
                </p>
              )}
            </div>
          </div>
